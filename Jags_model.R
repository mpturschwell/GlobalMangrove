...
mangroves <- read.csv("GlobalMangroveData_MARCH5.csv", header = T)


require(tibble)
AreaLost_NATION <- tapply(mangroves$area_loss, mangroves$NATION, sum)
ALN <- enframe(AreaLost_NATION)
ALN <- ALN[order(-ALN$value),]
top10_arealoss <- ALN[1:15,]

mangroves$notprotectedpercentage[mangroves$notprotectedpercentage<0] <- 0

# select cells with GDP info 
#mangroves <- mangroves[!is.na(mangroves["GDP_trend_1990.2000"]),]
#mangroves <- mangroves[!is.na(mangroves["GDP_trend_2000.2012"]),]
#mangroves <- mangroves[!is.na(mangroves["GDP_trend_1990.2012"]),]

#mangroves <- mangroves[!is.na(mangroves["GDP_mean_1990.2000"]),]
mangroves <- mangroves[!is.na(mangroves["GDP_mean_2000.2012"]),]
#mangroves <- mangroves[!is.na(mangroves["GDP_mean_1990.2012"]),]

#mangroves <- mangroves[!is.na(mangroves["HDI_2000.1"]),]
...
# select cells with HDI info
#mangroves <- mangroves[!is.na(mangroves["HDI_2000"]),]
...
# try CLUMPY, AREA_MN, AREA_AM, PLAND
...
#eg <- mangroves[which(mangroves$NATION=='EQUATORIAL GUINEA'),]
#mangroves <- mangroves[!(mangroves$NATION %in% eg$NATION),]
...
#GDP <- as.numeric(unique(log(mangroves$mean_1990.2000)))

#GDP <- as.numeric(tapply(mangroves$SCGDP_trend_1990.2000, mangroves$NATION, mean))
#GDP <- as.numeric(tapply(mangroves$SCGDP_trend_2000.2012, mangroves$NATION, mean))
#GDP <- as.numeric(tapply(mangroves$SCGDP_trend_1990.2012, mangroves$NATION, mean))

#GDP <- as.numeric(tapply(mangroves$SCGDP_mean_1990.2000, mangroves$NATION, mean))
GDP <- as.numeric(tapply(mangroves$SCGDP_mean_2000.2012, mangroves$NATION, mean))
#GDP <- as.numeric(tapply(mangroves$SCGDP_mean_1990.2012, mangroves$NATION, mean))

#GDP <- as.numeric(tapply(mangroves$HDI_2000.1, mangroves$NATION, mean))

parameters <- list("mu", "b","g","tau", "sigma") # add sigma in too
...
# Plot MCMC chains 
#
#jagsfit.mcmc <- as.mcmc(Rjags.mod)
#par(mfrow = c(2,2))
#plot(jagsfit.mcmc)
...
setwd("C:/POSTDOC_2018-2021/Mangrove_trends/Data/Sebastian/Model_plots/May2019")
...
> logGDP <- log(mangroves$GDP_mean_2000.2012)
...
#  xlim(-1,15) +
  ...
  xlab("Log mean GDP 2000-2012")+
  ylab("Log Proportion of Mangroves Area in 2012 relative to 2000")
q
ggsave("Relationship between mean GDP 2000-2012 and estimated Mangrove loss.png", q, width = 12, height = 8, units = c("in"), dpi = 300)
